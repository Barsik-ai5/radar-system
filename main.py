...
Правила:
1. МУСОР = ИГНОР. СТРОГО: Если в тексте есть фразы "угроза сохраняется", "опасность сохраняется", сборы средств, реклама или пожелания ночи — верни ТОЛЬКО слово ИГНОР.
2. СВЕТОФОР ЭМОДЗИ:
   🔴 [КРАСНЫЙ] — Прямая угроза: Ракетная/авиационная опасность, атака FPV, УАБ, ТРЕВОГА по БПЛА, падение обломков. ВАЖНО: Любое упоминание "УДАРНЫХ БПЛА" (даже их "пролет" или "движение") — это СТРОГО 🔴 КРАСНЫЙ!
   ⚪️ [БЕЛЫЙ] — Инфо: Последствия атак, разрушения, сбитые цели.
   🟡 [ЖЕЛТЫЙ] — Предупреждение: ОПАСНОСТЬ БПЛА, фиксация/движение ОБЫЧНЫХ БПЛА (если не сказано, что они ударные).
   🟢 [ЗЕЛЕНЫЙ] — Отбой: Отбой любой опасности.
3. ОПРЕДЕЛИ РЕГИОН. Мы отслеживаем ТОЛЬКО: Питер/Ленобласть -> [SPB], Москва/МО -> [MSK], Белгородская обл -> [BELGOROD], Курск/Курская обл -> [KURSK].
4. 🚨 ПРАВИЛО МАССОВЫХ ТРЕВОГ (ВЫРЕЗАЙ ЛИШНЕЕ):
   Если в тексте перечислены регионы (Брянская, Орловская, Смоленская, Курская, Калужская и т.д.):
   ТВОЯ ЗАДАЧА — ВЫРЕЗАТЬ все чужие регионы! Оставь ТОЛЬКО наш целевой регион.
   ПРИМЕР: Текст "Брянская, Калужская, Курская области - опасность БПЛА" -> Твой ответ: "[KURSK] 🟡 Курская область - опасность по БПЛА."
   Если наших регионов нет вообще — верни ИГНОР.
5. Верни ТОЛЬКО тег и текст. Не обрезай названия городов нашего региона.
"""
...
async def radar_handler(client, message):
    ...
    # === ЖЕСТКАЯ ПРИВЯЗКА КАНАЛОВ К РЕГИОНАМ (ЧТОБЫ НЕ БЫЛО ДУБЛЕЙ) ===
    if "locator" in source_lower:
        allowed_regions = ["KURSK"]
    elif "vrv" in source_lower:
        allowed_regions = ["SPB", "MSK"]
    else:
        # Для radar_ru_belgorod
        allowed_regions = ["BELGOROD"]

    is_sent = False

    # 1. Отправка СТРОГО по разрешенным для этого канала тегам
    ...
    # 2. АВАРИЙНЫЙ СБРОС (Если ИИ забыл тег, мы знаем куда кидать по источнику)
    if not is_sent:
        if "locator" in source_lower:
