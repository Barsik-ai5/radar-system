...
Правила:
1. МУСОР = ИГНОР. Если это общие слова, фразы "угроза сохраняется", сборы средств, пожелания ночи или РЕКЛАМА (особенно фейковые тревоги с призывами "подписаться", "канал вашего города", "без связи") — верни ТОЛЬКО слово ИГНОР.
2. ВАЖНО: Выдели главное, сохранив все названия населенных пунктов. Не обрезай текст слишком сильно. Отвечай СТРОГО НА РУССКОМ ЯЗЫКЕ! 
   РАССТАВЬ ЭМОДЗИ СТРОГО ПО ЭТОЙ ЛОГИКЕ (СВЕТОФОР):
   🔴 [КРАСНЫЙ] — Прямая угроза: Ракетная, бомбовая, авиационная опасность, атака FPV/ФПВ, УАБ, баллистика, ТРЕВОГА по БПЛА, массовые/ударные БПЛА, падение обломков.
   ⚪️ [БЕЛЫЙ] — Последствия/Инфо: Последствия атак, разрушения на земле, информация о сбитых целях
   🟡 [ЖЕЛТЫЙ] — Предупреждение: ОПАСНОСТЬ БПЛА, фиксация БПЛА, движение БПЛА в направлении, режим внимания, потенциальная угроза.
   🟢 [ЗЕЛЕНЫЙ] — Отбой: Отбой любой опасности, ракетной или БПЛА. (СООБЩЕНИЯ ОБ ОТБОЕ ПУБЛИКОВАТЬ ОБЯЗАТЕЛЬНО!).
3. ОПРЕДЕЛИ РЕГИОН. Мы отслеживаем ТОЛЬКО эти:
   - Питер/Ленобласть -> [SPB]
   - Москва/МО -> [MSK]
   - Белгородская обл -> [BELGOROD]
   - Курск/Курская обл -> [KURSK]
4. 🚨 ПРАВИЛО МАССОВЫХ ТРЕВОГ (ОЧЕНЬ ВАЖНО):
   Если в тексте ЕСТЬ наш регион (например, Московская обл), но рядом перечислены чужие (Брянск, Воронеж, Орел, Липецк и т.д.) — НЕ ПИШИ ИГНОР! 
   Вместо этого ВЫРЕЖИ чужие регионы из текста. Оставь только НАШ регион и суть угрозы/отбоя для него, и выдай наш тег.
   Слово ИГНОР возвращай ТОЛЬКО в том случае, если наших регионов в тексте НЕТ ВООБЩЕ.
5. Верни ТОЛЬКО тег(и) и готовый текст.
"""
...
async def radar_handler(client, message):
    ...
    print(f"[*] Чистый ответ от ИИ: {result}") 

    clean_text = re.sub(r'\[.*?\]\s*', '', result).strip()
    
    all_target_regions = ["SPB", "MSK", "BELGOROD", "KURSK"]
    is_sent = False

    # 1. Отправка по тегам (здесь используется {region})
    for region in all_target_regions:
        if f"[{region}]" in result or f"[{region.lower()}]" in result.lower():
            await loop.run_in_executor(None, send_to_channel, region, clean_text)
            print(f"[+] Успешно отправлено в {region} (по тегу ИИ)!")
            is_sent = True

    # 2. АВАРИЙНЫЙ СБРОС (здесь жестко прописаны регионы, так как тега нет)
    if not is_sent:
        if "locator" in source_lower:
            await loop.run_in_executor(None, send_to_channel, "KURSK", clean_text)
            print(f"[+] Отправлено в KURSK принудительно (ИИ забыл тег)")
        elif "vrv" in source_lower:
            if "моск" in text.lower() or "ступ" in text.lower():
                await loop.run_in_executor(None, send_to_channel, "MSK", clean_text)
                print(f"[+] Отправлено в MSK принудительно (ИИ забыл тег)")
        else:
            await loop.run_in_executor(None, send_to_channel, "BELGOROD", clean_text)
            print(f"[+] Отправлено в BELGOROD принудительно (ИИ забыл тег)")
